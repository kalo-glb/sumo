...
#define detect_in_back() (!detect_rear_center)
...
// motion functions will be executed only if needed
typedef enum md
{
    Forward,
    Backward,
...
void forward()
{
    if(Forward != mode)
    {
      ...
      push_rear_left.writeMicroseconds(1000);
      push_rear_right.writeMicroseconds(2000);
...
void backward()
{
    if(Backward != mode)
    {
      ...
      turn_servo.write(TurnForward);
      push_rear_left.writeMicroseconds(2000);
      push_rear_right.writeMicroseconds(1000);
      mode = Backward;
    }
}
...
int detect_end_of_ring()
{
  if (detect_line_fl() && detect_line_fr()){
    ...
    // fix for choppy motion when going backwards
    if(!detect_in_back())
    {
        forward();
    }
